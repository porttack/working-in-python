...
| 2. Chapter surgery | `mods/pass-2-surgery.md` | destructive edits + authoring | in progress — ch. 1–8 done (the whole "chapters 3–8" batch, across three sessions). Ch. 9–19: Step 1 (VA removal) and Step 2 (replacement exercise) done 2026-09-09 for all eleven chapters in one batch, following the book-wide VA-removal policy decided the same day (see the treatment matrix above and `AUDIT.md`'s 2026-09-09 handoff). This closes out the book's kind-A count at four-for-four: ch17-ex07 (Kangaroo mutable-default-argument bug) was the last one standing and now has a native replacement, ch17-ex07r. Step 4 (blank markers) then done the same day for chapters 9–13 (the ones certain to matter — see `AUDIT.md`'s follow-up handoff; 14–19 stay at `none` regardless of assignment plans since that tier is about live/projected teaching, not assignment — see the scope-uncertainty project memory). Per-chapter checkout (Step 5) for ch. 9–13 is still pending, and chapters 14–19 remain fully untouched. Three of the four kind-A replacements before the VA-removal work stood as written; the fourth (ch05-ex06's replacement, ch05-ex07, a Sierpinski-triangle turtle exercise) was written, then removed 2026-08-24 as too failure-prone for ungraded optional practice with no in-class reference solution -- that slot still has no native replacement (unrelated to the ch17 work). See `data/exercise-ledger.json` and `AUDIT.md`. Pacing question still open. `chap06b`, the first *interlude* (original content between two numbered chapters — see `CHAPTER_MANIFEST.md`), authored outside this pass's normal order and fully wired 2026-08-17; more interludes planned (unit testing, binary). `chap07b`, the second interlude ("Representing Data," between chapters 7 and 8), supplied 2026-08-17 as an outline only — no drafted prose or exercises yet — and wired into the TOC and vocab/standards back matter ahead of authoring; still needs actual Pass 2 drafting. See `AUDIT.md` |
| 3. Standards alignment | `mods/pass-3-alignment.md` | analysis + additive back matter | in progress — Steps 1–3 done (both indexes, `standards/crosswalk.json`, all three `alignment/*.md` docs); Step 4 done for chapters 1–13, all four frameworks (AP CSP, CA 9-12, CSTA 2026, CA CTE/ICT). Chapters 9–13's turn was 2026-09-09: the actual alignment research now happens in a separate repo, `../learn` (canonical source of truth for every content source the maintainer teaches from, not just this book — see its own `_standards/README.md`); this repo's `standards/` is a synced, read-only mirror (`learn/tools/sync-standards.sh`), and `alignment/*.html`/`alignment/standards-alignment.md` (renamed from `standards_alignment.md` the same day) are generated from that mirror by `tools/build_alignment.py`, same as before. The site's own "Reference" nav (`jb/_toc.yml`) now points its standards-catalog entries at `learn.porttack.com` directly rather than this repo's local copies (maintainer decision, scoped to the nav only — per-chapter blocks still cite the local `alignment/*.html` anchors). Chapters 14–19 and Step 5 not started. AP 2.1/2.2 and CA DA.8/DA.9 carriers reassigned to `chap07b` 2026-08-17, ahead of that chapter actually being drafted — see `AUDIT.md`. CSTA 2026 and CA CTE/ICT for `chap07b` still TBD. See `AUDIT.md` |
| 4. Chapter chrome | `mods/pass-4-chrome.md` | destructive edits (front/back matter only — link bar, embedded pane, exercises notebook, attribution rule) | in progress — ch. 1–2 done, built up across several rounds of live testing against the deployed site; ch. 3–8 done 2026-08-09, matched the pattern exactly (ch. 8 needed a `CELL_PATCHES` merge, not a fresh key — see `AUDIT.md`); ch. 9–13 done 2026-09-09, one batch, chrome scope deliberately extended from 1–11 to 1–13 that same day (maintainer decision — see `AUDIT.md`); ch. 14–19 out of scope (independent study, no live/projected component). chap12 and chap13 needed fresh `CHAPTERS` entries in `tools/build_jupyterlite_content.py` (not registered before); chap13 needed three additional `CELL_PATCHES` fixes for unhandled `!` shell-magic lines (`pip install`, `unzip`, `rm`), beyond the standard self-embed one every chrome-bearing chapter gets. `chap06b` interlude got the same chrome treatment 2026-08-17, plus a `jb/_toc.yml`/`jb/build.sh` wrinkle unique to interludes — see `AUDIT.md`. `chap07b` (second interlude, outline only) deliberately has NOT had this pass's treatment yet — no chrome for a chapter with no prose — pending its actual authoring. Step 1 (Homework-section content authoring, superseded from the old separate-notebook pattern 2026-08-16) is its own tracked sub-progress: chapters 1, 2, 4, 5, 6, and 6b have a `## Homework` section; chapter 8 got its first one 2026-09-07 and chapter 7 got its first one 2026-09-08 (both out of order, same-day assignments); chapters 9–13 all got theirs 2026-09-09, one batch, following chapters 7/8's exact pattern (five numbered Problems, a choose-one pair for Problems 1–2, a debug problem, a reflection, a time check, and 0.5-point extra credit) — every number quoted in these five chapters' problems (word counts, letter frequencies, longest words, a common bigram, file sizes) was verified by actually running the relevant code against the chapter's own vendored data (`words.txt`, `pg43.txt`, `photos.zip`) before being written into a docstring or prose, not estimated. chapter 12 had no `run_doctests` helper at all before this (its practice exercises don't use doctest), so the standard three-line definition was added as the Homework section's own first cell. See `AUDIT.md`. **Terminology split, deliberate:** chapters 7, 8, and now 9–13 number their graded Homework items "Problem N" (renamed from "Exercise N" 2026-09-08, to stop that word meaning two different things depending on section); chapters 1, 2, 4, 5, 6, and 6b still say "Exercise N" in their own Homework sections — not yet reconciled, see `AUDIT.md` for the scoping call. |
| 5. JupyterLite lab & naming | `mods/pass-5-jupyterlite-lab.md` | additive tooling (`chapters/index.ipynb`, `CONTENT_NAMES`, `overrides.json`, `custom.js`) + a small piece of every future Pass 2/4 chapter | in progress — front page, naming foundation for everything currently in `CHAPTERS`, and sticky `?readonly` nav done 2026-08-09; `CONTENT_NAMES` pre-populated for ch. 9–19 ahead of Passes 2/4 reaching them. Ch. 9–13 now wired into `CHAPTERS` as of 2026-09-09's chrome work (Pass 4); ch. 14–19 still pre-populated in `CONTENT_NAMES` only, not wired in, and won't be unless chrome scope is extended again. `chap07b` (second interlude, outline only as of 2026-08-17) deliberately NOT added to `CHAPTERS`/`CONTENT_NAMES` yet — pending actual authoring, same reasoning as Pass 4. See `AUDIT.md` |
