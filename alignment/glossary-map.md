...
| docstring | keep | Python's realization of AP's general "program documentation" concept (CRD-2.G) — the idea is tested, the term isn't. Interlude A states this mapping to students directly, in its own glossary. |
| doctest, test case, boundary case, edge case, expected value, pass/fail, hand tracing | keep | Python/this-book's testing vocabulary, introduced by Interlude A. Boundary/edge case and hand tracing realize AP's own "test case" and manual-tracing expectations (CRD-2 written-response language) without AP requiring these exact words; doctest, pass/fail, and expected value are Python/tooling-specific and not exam vocabulary at all. |
...
| regression | keep | professional testing vocabulary, introduced (and explicitly flagged as not exam vocabulary) by Interlude A. |
| compression ratio, run-length encoding | keep | this book's own vocabulary for the lossless/lossy contrast, introduced (and explicitly flagged as not exam vocabulary) by Interlude B, "Representing Data." The exam tests the *choice* between lossless and lossy compression, not any specific algorithm or metric — these two words make that choice measurable and concrete rather than purely verbal. As of 2026-08-17 that interlude is an outline only, not drafted prose. |
