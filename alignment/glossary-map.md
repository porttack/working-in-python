...
| compression ratio, run-length encoding | keep | this book's own vocabulary for the lossless/lossy contrast, introduced (and explicitly flagged as not exam vocabulary) by the interlude between chapters 7 and 8, "Representing Data." The exam tests the *choice* between lossless and lossy compression, not any specific algorithm or metric — these two words make that choice measurable and concrete rather than purely verbal. As of 2026-08-17 that interlude is an outline only, not drafted prose. |
